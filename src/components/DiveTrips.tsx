...
// Primamo 'data' kao prop iz App.tsx
export default function DiveTrips({ data }: { data: DiveTrip[] }) {
  const navigate = useNavigate();

  // 1. KLJUČNO: Ako nema podataka, komponenta vraća NULL i potpuno nestaje sa ekrana
  if (!data || data.length === 0) return null;

  return (
    <section id="zaroni" className="w-full bg-ocean-light py-24 px-6 md:px-12 lg:px-24 scroll-mt-24">
      <div className="w-full relative">
        ...
        <Swiper
          ...
          className="!pb-10"
        >
          {/* 2. KORISTIMO 'data' direktno umesto lokalnog state-a 'trips' */}
          {data.map((trip) => {
              ...
              <SwiperSlide key={trip.id} className="h-auto">
                <div className="group bg-white rounded-[2.5rem] overflow-hidden shadow-card hover:shadow-2xl transition-all duration-500 flex flex-col h-full border border-gray-100">
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    ...
                    <button 
                      onClick={() => {
                        navigate(`/rezervacija/${documentId}`, { state: { tripData: trip } });
                        window.scrollTo(0, 0);
                      }}
